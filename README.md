...
```json
{
  ...
  "segments": [
    ...
    {"name": "晚上", "start": "18:30", "end": "22:00"},
    {"name": "早上", "start": "08:00", "end": "12:30"}
  ]
}
```
...
- `seats`：按优先级排列的座位号列表
- `segments`：时段列表，**按预约优先级排列**
  - **第一个时段为必选段**（如下午）：预约失败则换下一个座位
  - **其余时段为可选段**（如晚上、早上）：预约失败则跳过该时段，继续下一个
  - 所有备选座位的必选段都失败时，自动随机查找当前房间任意可用座位
- `useCaptcha`：是否开启验证码自动识别（默认 true）

**预约逻辑示例**（segments = [下午, 晚上, 早上]）：
1. 尝试座位 711 的下午段 → 成功
2. 尝试座位 711 的晚上段 → 失败，跳过
3. 尝试座位 711 的早上段 → 成功
4. 完成：座位 711 约了下午 + 早上
